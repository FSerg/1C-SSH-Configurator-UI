...
UI обертка на Streamlit + простой HTTP API для удалённой работы с агентом конфигуратора 1С через SSH. Позволяет управлять выгрузкой/загрузкой основной конфигурации, расширений и внешних отчётов/обработок, видеть ход выполнения в JSON‑режиме и хранить набор подключений (Проекты) локально.
...
- Опционально: сохранение подключения к БД между операциями и ручное управление подключением (кнопки в сайдбаре). Настраиваемый таймаут простоя SSH‑канала.
...
- В настройках проекта укажите:
  ...
  - Включите «Сохранять подключение к БД», если хотите выполнять серии операций без отключения, и задайте «Таймаут бездействия SSH‑канала, сек».
